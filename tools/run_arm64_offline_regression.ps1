...
function Test-BattleBeforeInitBattleGetAddMaxMpShape([string]$repoRootWsl, [string]$bytecodeWsl, [string]$disPath) {
    $disWsl = Convert-ToWslPath $disPath
    $dumpCmd = @(
        "cd '$repoRootWsl'",
        "./tools/bc_dump_proto_wsl '$bytecodeWsl' 19 1299 1362 > '$disWsl'"
    ) -join " && "
    $code = Invoke-WslBash $dumpCmd
    if ($code -ne 0) {
        return [pscustomobject]@{
            ok = $false
            failures = @("bc_dump_proto_wsl proto19 1299..1362 failed exit=$code")
        }
    }
    ...
    $checks = @(
        @{ pc = 1299; op = "CALL";  a = 11; b = 2;  c = 1;   d = 513;  tag = "pc1299" },
        ...
        @{ pc = 1356; op = "MOV";   a = 17; b = 0;  c = 0;   d = 0;    tag = "pc1356" },
        @{ pc = 1357; op = "TGETS"; a = 15; b = 0;  c = 174; d = 174;  tag = "pc1357" },
        @{ pc = 1358; op = "KSTR";  a = 18; b = 0;  c = 175; d = 175;  tag = "pc1358" },
        @{ pc = 1359; op = "MOV";   a = 19; b = 0;  c = 11;  d = 11;   tag = "pc1359" },
        @{ pc = 1360; op = "KSTR";  a = 20; b = 0;  c = 176; d = 176;  tag = "pc1360" },
        @{ pc = 1361; op = "CAT";   a = 18; b = 18; c = 20;  d = 4628; tag = "pc1361" },
        @{ pc = 1362; op = "CALL";  a = 15; b = 1;  c = 3;   d = 259;  tag = "pc1362" }
    )
...
function Test-GameEngineJianghuContentUnpackWindow([string]$repoRootWsl, [string]$bytecodeWsl, [string]$disPath) {
    $disWsl = Convert-ToWslPath $disPath
    $dumpCmd = @(
        "cd '$repoRootWsl'",
...
function Test-AttackLogicExtendTalents2MissLogWindow([string]$repoRootWsl, [string]$bytecodeWsl, [string]$disPath) {
    ...
    $goodPattern = '(?ms)^\d{4}\s+line=6346\s+MOV\s+A=12\s+B=0\s+C=3\s+D=3.*\r?\n' +
        ...
        '^\d{4}\s+line=6346\s+CALL\s+A=10\s+B=1\s+C=3\s+D=259'
    $badPattern2 = '(?ms)^\d{4}\s+line=6350\s+MOV\s+A=11\s+B=0\s+C=3\s+D=3.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=10\s+B=3\s+C=26\s+D=794.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=12\s+B=0\s+C=31\s+D=31.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=13\s+B=0\s+C=27\s+D=27.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=13\s+B=13\s+C=4\s+D=3332.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=14\s+B=0\s+C=32\s+D=32.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=15\s+B=1\s+C=27\s+D=283.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=15\s+B=15\s+C=4\s+D=3844.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=16\s+B=0\s+C=33\s+D=33.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=17\s+B=5\s+C=29\s+D=1309.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=18\s+B=0\s+C=34\s+D=34.*\r?\n' +
        '^\d{4}\s+line=6350\s+CAT\s+A=12\s+B=12\s+C=18\s+D=3090.*\r?\n' +
        '^\d{4}\s+line=6350\s+CALL\s+A=10\s+B=1\s+C=3\s+D=259'
    $goodPattern2 = '(?ms)^\d{4}\s+line=6350\s+MOV\s+A=12\s+B=0\s+C=3\s+D=3.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=10\s+B=3\s+C=26\s+D=794.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=13\s+B=0\s+C=31\s+D=31.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=14\s+B=0\s+C=27\s+D=27.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=14\s+B=14\s+C=4\s+D=3588.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=15\s+B=0\s+C=32\s+D=32.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=16\s+B=1\s+C=27\s+D=283.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=16\s+B=16\s+C=4\s+D=4100.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=17\s+B=0\s+C=33\s+D=33.*\r?\n' +
        '^\d{4}\s+line=6350\s+TGETS\s+A=18\s+B=5\s+C=29\s+D=1309.*\r?\n' +
        '^\d{4}\s+line=6350\s+KSTR\s+A=19\s+B=0\s+C=34\s+D=34.*\r?\n' +
        '^\d{4}\s+line=6350\s+CAT\s+A=13\s+B=13\s+C=19.*\r?\n' +
        '^\d{4}\s+line=6350\s+CALL\s+A=10\s+B=1\s+C=3\s+D=259'
    $badCount = [regex]::Matches($disText, $badPattern).Count
    $goodCount = [regex]::Matches($disText, $goodPattern).Count
    ...
    if ($badCount2 -ne 0) {
        $fails.Add("residual bad proto320 extendtalents2 realdmg bf.Log window")
    }
    if ($goodCount2 -eq 0) {
        $fails.Add("missing corrected proto320 extendtalents2 realdmg bf.Log window")
    }
...
function Test-AttackLogicShieldLogWindow([string]$repoRootWsl, [string]$bytecodeWsl, [string]$disPath) {
    $disWsl = Convert-ToWslPath $disPath
    $dumpCmd = @(
        "cd '$repoRootWsl'",
...
function Test-AttackLogicExtendTalents2TalentLogWindows([string]$repoRootWsl, [string]$bytecodeWsl, [string]$outDir) {
    $targets = @(
        ...
        @{
            proto = 320
            dis = (Join-Path $outDir "attacklogic.proto320.extendtalents2_log_line6583.dis.txt")
            bad = '(?ms)^\d{4}(?:\s+line=\d+)?\s+MOV\s+A=21\s+B=0\s+C=3\s+D=3.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=20\s+B=3\s+C=26\s+D=794.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=22\s+B=1\s+C=27\s+D=283.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=22\s+B=22\s+C=4\s+D=5636.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+KSTR\s+A=23\s+B=0\s+C=139\s+D=139.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+MOV\s+A=24\s+B=0\s+C=17\s+D=17.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+KSTR\s+A=25\s+B=0\s+C=140\s+D=140.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+CAT\s+A=22\s+B=22\s+C=25\s+D=5657.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+CALL\s+A=20\s+B=1\s+C=3\s+D=259'
            good = '(?ms)^\d{4}(?:\s+line=\d+)?\s+MOV\s+A=22\s+B=0\s+C=3\s+D=3.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=20\s+B=3\s+C=26\s+D=794.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=23\s+B=1\s+C=27\s+D=283.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+TGETS\s+A=23\s+B=23\s+C=4\s+D=5892.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+KSTR\s+A=24\s+B=0\s+C=139\s+D=139.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+MOV\s+A=25\s+B=0\s+C=17\s+D=17.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+KSTR\s+A=26\s+B=0\s+C=140\s+D=140.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+CAT\s+A=23\s+B=23\s+C=26\s+D=5914.*\r?\n' +
                '^\d{4}(?:\s+line=\d+)?\s+CALL\s+A=20\s+B=1\s+C=3\s+D=259'
        }
    )
...
function Test-AttackLogicExtendTalents2RealDmgLogHarness([string]$repoRootWsl, [string]$bytecodeWsl, [string]$logPath) {
    $logWsl = Convert-ToWslPath $logPath
    $runCmd = @(
        "cd '$repoRootWsl'",
        "./tools/bcrun_cli_wsl '$bytecodeWsl' attacklogic_extendtalents2_realdmglog > '$logWsl' 2>&1"
    ) -join " && "
    $code = Invoke-WslBash $runCmd
    return [pscustomobject]@{
        ok = ($code -eq 0)
...
foreach ($name in $targets) {
    ...
    if ($name -eq "battle.lua") {
        ...
        $registerPrevRoleDis = Join-Path $outAbs "battle.proto19.beforeinitbattle_getaddmaxmp.dis.txt"
        $registerPrevRoleShapeCheck = Test-BattleBeforeInitBattleGetAddMaxMpShape -repoRootWsl $repoRootWsl -bytecodeWsl $outWsl -disPath $registerPrevRoleDis
        $registerPrevRoleShapeFailures = @($registerPrevRoleShapeCheck.failures).Count
        if (-not $registerPrevRoleShapeCheck.ok) {
            $failed = $true
            foreach ($msg in $registerPrevRoleShapeCheck.failures) {
                Write-Warning "[battle beforeinitbattle getaddmaxmp shape] $msg"
            }
        }
        ...
        $registerPrevRoleHarnessFailures = if ($registerPrevRoleCheck.ok) { 0 } else { 1 }
        $registerPrevRoleFailures = $registerPrevRoleShapeFailures + $registerPrevRoleHarnessFailures
...
if (Test-Path $gameEnginePath) {
    ...
    $gameEngineJianghuDis = Join-Path $outAbs "gameengine.proto112.jianghucontent_unpack.dis.txt"
    $gameEngineJianghuCheck = Test-GameEngineJianghuContentUnpackWindow -repoRootWsl $repoRootWsl -bytecodeWsl $outWsl -disPath $gameEngineJianghuDis
    if ($code -ne 0 -or $convFail -gt 0 -or $converted -eq 0 -or -not $gameEngineCheck.ok -or -not $gameEngineJianghuCheck.ok) {
        $failed = $true
    }
    ...
    if (-not $gameEngineJianghuCheck.ok) {
        foreach ($msg in $gameEngineJianghuCheck.failures) {
            Write-Warning "[gameengine jianghu unpack shape] $msg"
        }
    }
}
...
if (Test-Path $attacklogicPath) {
    ...
    if (-not $attackLogicShieldLogCheck.ok) {
        $failed = $true
        foreach ($msg in $attackLogicShieldLogCheck.failures) {
            Write-Warning "[attacklogic shield bf.log shape] $msg"
        }
    }
    ...
    $extendTalents2RealDmgLog = Join-Path $outAbs "attacklogic.extendtalents2.realdmglog.log"
    $extendTalents2RealDmgHarnessCheck = Test-AttackLogicExtendTalents2RealDmgLogHarness -repoRootWsl $repoRootWsl -bytecodeWsl $outWsl -logPath $extendTalents2RealDmgLog
    $extendTalents2RealDmgHarnessFailures = if ($extendTalents2RealDmgHarnessCheck.ok) { 0 } else { 1 }
