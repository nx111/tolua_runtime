...
| `battle.lua` | `RegisterWorkflowForSkills` | 字符串拼接参与者应为 string | 本地反汇编通过（3580/3585/3589 参数统一右移），待真机 |
...
1. 真机回归 `migong.lua` 与 `battle.lua` 这两个主线点。  
2. 若仍报错，先把“首条报错堆栈 + 文件行号”回写到本日志再改代码。  
3. 回归通过后，冻结本轮规则，不再继续泛化。  
