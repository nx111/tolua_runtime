...
static int run_battle_rest_harness(lua_State *L)
{
  const char *harness =
      ...
      "  for i = 1, #items do t[i - 1] = items[i]; t[i] = items[i] end\n"
      "  return t\n"
      "end\n"
      "local function zero_list() return list({}) end\n"
      "function LuaTool.CreateLuaTable(v)\n"
...
static int run_battle_rest_huanhun_mincall_harness(lua_State *L)
{
  const char *harness =
      ...
      "assert(string.find(tostring(err), '__HUANHUN_MIN_OK__', 1, true) ~= nil, 'huanhun err=' .. tostring(err))\n";
...
static int run_battle_beforeskillanimation_wushuanglog_harness(lua_State *L)
{
  const char *harness =
      "RuntimeData.Instance = RuntimeData.Instance or {}\n"
      "RuntimeData.Instance.gameEngine = { CurrentSceneValue = '', battleType = '' }\n"
      ...
      "  assert(type(msg) == 'string', 'bf.Log msg=' .. type(msg))\n"
      "  if string.find(msg, '【无双之力】发动，获得了', 1, true) ~= nil then\n"
      "    error('__BSA_WUSHUANG_OK__ msg=' .. msg, 0)\n"
      "  end\n"
      "end\n"
      "local sprite = {\n"
      ...
      "function sprite:AddBuff(name, level, round)\n"
      "  assert(self == sprite, 'sprite.AddBuff self=' .. tostring(self))\n"
      "  assert(name == '无双', 'sprite.AddBuff name=' .. tostring(name))\n"
      "  assert(type(level) == 'number', 'sprite.AddBuff level=' .. type(level))\n"
      "end\n"
      "function sprite:AddBuffOnly2() end\n"
      "function sprite:Say() end\n"
      "bf.SpritesTable = { sprite }\n"
      ...
      "local callback = wf.callbacks['无双之力']\n"
      "assert(type(callback) == 'function', 'wf.callbacks[无双之力]=' .. type(callback))\n"
      "local prev = {}\n"
      "local skill = { Name = 'offline-skill', IsUnique = true, CostBall = 3 }\n"
      "local ok, err = pcall(callback, bf, sprite, skill, 1, prev)\n"
      "assert(ok == false, 'BeforeSkillAnimation callback returned unexpectedly')\n"
      "assert(string.find(tostring(err), '__BSA_WUSHUANG_OK__', 1, true) ~= nil, 'BeforeSkillAnimation callback err=' .. tostring(err))\n";
...
static int run_battle_beforeskillanimation_shuangbingqilog_harness(lua_State *L)
{
  const char *harness =
      ...
      "prevRole.skillSpecials[skill.Name] = { 双持 = 100 }\n"
      "table.insert(prevRole.workflows.BeforeSkillAnimation, callback)\n"
      "local ok, err = pcall(BATTLE_BeforeSkillAnimation, bf, sprite, skill, 1)\n"
      "assert(ok == false, 'BATTLE_BeforeSkillAnimation 双兵器 returned unexpectedly')\n"
      "assert(string.find(tostring(err), '__BSA_SHUANGBINGQI_OK__', 1, true) ~= nil, 'BATTLE_BeforeSkillAnimation 双兵器 err=' .. tostring(err))\n";
...
static int run_battle_beforeroleaction_mincall_harness(lua_State *L)
{
  const char *harness =
      ...
      "  return raw_min(a, b, ...)\n"
      "end\n"
      "local raw_max = math.max\n"
      "math.max = function(a, b, ...)\n"
      "  if type(a) ~= 'number' or type(b) ~= 'number' then\n"
      "    error('__BRA_MAX_BAD__ a=' .. type(a) .. ':' .. tostring(a) .. ' b=' .. type(b) .. ':' .. tostring(b), 0)\n"
      "  end\n"
      "  return raw_max(a, b, ...)\n"
      "end\n"
...
static int run_battle_beforeroleaction_buffprobe_harness(lua_State *L)
{
  const char *harness =
      ...
      "    end\n"
      "    local bf2, sprite2, prevRole2 = make_case()\n"
      "    sprite2.Role.Level = 1\n"
      "    sprite2.Mp = 4000\n"
      "    sprite2.MaxMp = 4000\n"
      "    sprite2.Role.anti_debuffValue = 1\n"
      "    local anti = sprite2:GetBuff('异种真气')\n"
      "    if anti then anti.Level = 1 end\n"
      "    local ok2, err2 = xpcall(function() return candidate(bf2, sprite2, prevRole2) end, debug.traceback)\n"
      "    if not ok2 and (\n"
      "      string.find(tostring(err2), 'sprite.GetBuff self=', 1, true) ~= nil or\n"
      "      string.find(tostring(err2), 'sprite.DeleteBuff self=', 1, true) ~= nil or\n"
      "      string.find(tostring(err2), 'sprite.HasBuff self=', 1, true) ~= nil or\n"
      "      string.find(tostring(err2), 'sprite.Set_needRefresh self=', 1, true) ~= nil) then\n"
      "      error('__BRA_BUFF_BAD__ talent=' .. tostring(talent) .. ' line=' .. tostring(info.linedefined or -1) .. ' case=antidebuff err=' .. tostring(err2), 0)\n"
      "    end\n"
      "  end\n"
      "end\n";
...
static int run_battle_rest_buffprobe_harness(lua_State *L)
{
  const char *harness =
      ...
      "    string.find(tostring(err), 'Set_needRefresh self=', 1, true) ~= nil or\n"
      "    string.find(tostring(err), 'bf.Log self=', 1, true) ~= nil or\n"
      "    string.find(tostring(err), 'bf.Log msg=', 1, true) ~= nil) then\n"
...
static int run_attacklogic_extendtalents2_fullprobe_harness_impl(lua_State *L, int misslog_mode, int realdmglog_mode, int shieldlog_mode, int xixing_mode, int remove_mode, int xilog_mode, int shenshui_mode, int shieldlog_direct_mode);
...
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 1, 0, 0, 0, 0, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_realdmglog_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 1, 0, 0, 0, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_shieldlog_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 1, 0, 0, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_shieldlog_direct_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 1, 0, 0, 0, 0, 1);
}
...
static int run_attacklogic_extendtalents2_fullprobe_harness_impl(lua_State *L, int misslog_mode, int realdmglog_mode, int shieldlog_mode, int xixing_mode, int remove_mode, int xilog_mode, int shenshui_mode, int shieldlog_direct_mode)
{
  ...
  if (shieldlog_mode) {
    setup = "__MISSLOG_EXPECT__ = false\n__REALDMGLOG_EXPECT__ = false\n__SHIELDLOG_EXPECT__ = true\n__SHIELDLOG_DIRECT_EXPECT__ = false\n__XIXING_EXPECT__ = false\n__REMOVE_EXPECT__ = false\n__XILOG_EXPECT__ = false\n__SHENSHUI_EXPECT__ = false";
    chunk_name = "@bcrun_attacklogic_extendtalents2_shieldlog_setup";
    run_chunk = "@bcrun_attacklogic_extendtalents2_shieldlog";
  }
  if (shieldlog_direct_mode) {
    setup = "__MISSLOG_EXPECT__ = false\n__REALDMGLOG_EXPECT__ = false\n__SHIELDLOG_EXPECT__ = true\n__SHIELDLOG_DIRECT_EXPECT__ = true\n__XIXING_EXPECT__ = false\n__REMOVE_EXPECT__ = false\n__XILOG_EXPECT__ = false\n__SHENSHUI_EXPECT__ = false";
    chunk_name = "@bcrun_attacklogic_extendtalents2_shieldlog_direct_setup";
    run_chunk = "@bcrun_attacklogic_extendtalents2_shieldlog_direct";
  }
  if (xixing_mode) {
  ...
  const char *harness =
      ...
      "local raw_probability_test = Tools.ProbabilityTest\n"
      "Tools.ProbabilityTest = function(...)\n"
      "  if _G.__SHENSHUI_EXPECT__ then return true end\n"
      "  if type(raw_probability_test) == 'function' then return raw_probability_test(...) end\n"
      "  return true\n"
      "end\n"
      "BattleUtil.FlagLianji = BattleUtil.FlagLianji or {}\n"
      "BattleUtil.PrevRoles = BattleUtil.PrevRoles or {}\n"
      ...
      "  if _G.__SHIELDLOG_EXPECT__ and string.find(msg, '的真气护盾减少', 1, true) ~= nil then error('__SHIELD_OK__') end\n"
      ...
      "  self.__attack_count = self.__attack_count + 1\n"
      "end\n"
      "local atk_talents = _G.__SHENSHUI_EXPECT__ and { '神水' } or ((_G.__REMOVE_EXPECT__ or _G.__XILOG_EXPECT__) and {} or (_G.__XIXING_EXPECT__ and { '吸星大法改' } or { '奇招怪式', '孤独求败' }))\n"
      "local def_talents = _G.__SHENSHUI_EXPECT__ and {} or ((_G.__REMOVE_EXPECT__ or _G.__XILOG_EXPECT__) and {} or (_G.__XIXING_EXPECT__ and {} or { '斗转星移', '心心相印' }))\n"
      "if _G.__EXTENDTALENTS_MAX_EXPECT__ then\n"
      ...
      "  local buffs = {}\n"
      "  local vars = { [0] = 0, [1] = 0 }\n"
      "  local buff_items = _G.__EXTENDTALENTS_MAX_EXPECT__ and { { Name = '离线负面', Level = 1, LeftRound = 1, Property = 0, IsDebuff = true } } or (_G.__SHENSHUI_EXPECT__ and { { Name = '真气紊乱', Level = 2, LeftRound = 3, Property = 0, IsDebuff = true } } or {})\n"
      "  local sprite = {\n"
      "    ParentBattleField = bf,\n"
      "    Role = role,\n"
      "    role = role,\n"
      ...
      "    for k in pairs(t) do rows[#rows + 1] = tostring(k) end\n"
      "    table.sort(rows)\n"
      "  end\n"
      ...
      "  local out = {}\n"
      "  if wf and type(wf.callbacks) == 'table' then\n"
      "    for talent, enabled in pairs(role_talents) do\n"
      "      if enabled then\n"
      ...
      "end\n"
      "if _G.__SHIELDLOG_EXPECT__ then\n"
      "  targetSprite.Shield = 100\n"
      "end\n"
      "if _G.__SHENSHUI_EXPECT__ then\n"
      "  targetSprite.Mp = 0\n"
      "  targetSprite.MaxMp = 0\n"
      "end\n"
      "atk_role.Sprite = sourceSprite\n"
      "def_role.Sprite = targetSprite\n"
      "bf.SpritesTable = { sourceSprite, targetSprite }\n"
      ...
      "end\n"
      "if _G.__SHENSHUI_EXPECT__ then\n"
      "  local shenshui_wf = BattleUtil.GetWorkflowForTalent('extendTalents2_forAttacker')\n"
      "  assert(type(prevRoleAtk.workflows.extendTalents2_forAttacker) == 'table', 'shenshui_attacker_flow=' .. type(prevRoleAtk.workflows.extendTalents2_forAttacker))\n"
      "  assert(#prevRoleAtk.workflows.extendTalents2_forAttacker >= 1, 'shenshui_attacker_callbacks=' .. tostring(#prevRoleAtk.workflows.extendTalents2_forAttacker) .. ' wf=' .. join_keys(shenshui_wf and shenshui_wf.callbacks) .. ' role=' .. join_keys(prevRoleAtk.roleTalents))\n"
      "end\n"
      ...
      "if not _G.__SHIELDLOG_DIRECT_EXPECT__ then\n"
      "  for upi = 1, 16 do\n"
      ...
      "  if _G.__SHIELDLOG_DIRECT_EXPECT__ then\n"
      "    local upname, upvalue = debug.getupvalue(AttackLogic_extendTalents2, 3)\n"
      "    if type(upvalue) ~= 'function' then error('shield helper missing up3=' .. tostring(type(upvalue)), 0) end\n"
      "    return upvalue(sourceSprite, targetSprite, bf, atk_skill, result, prevRoleAtk, prevRoleDef)\n"
      "  end\n"
      ...
      "elseif _G.__SHIELDLOG_EXPECT__ then\n"
      "  assert(ok == false, 'extendtalents2 returned before shield log')\n"
      "  assert(string.find(tostring(ret), '__SHIELD_OK__', 1, true) ~= nil, 'extendtalents2 err=' .. tostring(ret))\n"
      ...
      "elseif _G.__SHENSHUI_EXPECT__ then\n"
      "  assert(ok == false, 'extendtalents2 returned before shenshui log')\n"
      "  assert(string.find(tostring(ret), '__SHENSHUI_OK__', 1, true) ~= nil, 'extendtalents2 err=' .. tostring(ret))\n"
      "else\n"
      "  assert(ok, ret)\n"
  ...
  return dostring(L, harness, run_chunk);
}

static int run_attacklogic_extendtalents2_fullprobe_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 0, 0, 0, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_xixingcallback_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 0, 1, 0, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_removeprobe_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 0, 0, 1, 0, 0, 0);
}

static int run_attacklogic_extendtalents2_xilog_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 0, 0, 0, 1, 0, 0);
}
...
static int run_attacklogic_extendtalents2_xiaoyaolog_harness(lua_State *L)
{
  const char *harness =
      ...
      "end\n"
      "local function make_role(name, key)\n"
      ...
      "    Skills = {},\n"
      "    SpecialSkills = {},\n"
      "    EquippedInternalSkill = { Name = '北冥神功', Level = 10 },\n"
      "    HiddenWeapon = '',\n"
      "    EquippedTitle = ''\n"
      "  })\n"
      "end\n"
      ...
      "  assert(self == bf, 'bf.Log self=' .. tostring(self))\n"
      "  assert(type(msg) == 'string', 'bf.Log msg=' .. type(msg))\n"
      "  error('__XIAOYAO_OK__')\n"
      "end\n"
      "local sourceRole = make_role('src-role', 'src-key')\n"
      "local targetRole = make_role('dst-role', 'dst-key')\n"
      "local sourceSprite = make_sprite(sourceRole, 1)\n"
      "local targetSprite = make_sprite(targetRole, 2)\n"
      ...
      "  AddAttackInfo = function() end\n"
      "})\n"
      "local prevRoleAtk = strictify('prevRoleAtk', {\n"
      "  talents = false_map(),\n"
      "  roleTalents = false_map(),\n"
      "  role = sourceRole,\n"
      "  Miss = 0\n"
      "})\n"
      "local xiaoyaoTalents = false_map({ ['逍遥之基'] = true })\n"
      "local prevRoleDef = strictify('prevRoleDef', {\n"
      "  talents = xiaoyaoTalents,\n"
      "  roleTalents = xiaoyaoTalents,\n"
      "  role = targetRole,\n"
      "  InternalSkillName = '北冥神功',\n"
      "  Miss = 0\n"
      "})\n"
      "local wf = BattleUtil.GetWorkflowForTalent('extendTalents2_forDefencer')\n"
      "assert(type(wf) == 'table', 'wf=' .. type(wf))\n"
      "local candidate = wf.callbacks and wf.callbacks['逍遥之基'] or nil\n"
      "assert(type(candidate) == 'function', 'candidate=' .. type(candidate))\n"
      "local ok, err = xpcall(function()\n"
      "  return candidate(sourceSprite, targetSprite, skill, bf, result, prevRoleAtk, prevRoleDef)\n"
      "end, debug.traceback)\n"
      "assert(ok == false, 'callback returned before xiaoyao log')\n"
      "assert(string.find(tostring(err), '__XIAOYAO_OK__', 1, true) ~= nil, 'err=' .. tostring(err))\n";

  return dostring(L, harness, "@bcrun_attacklogic_extendtalents2_xiaoyaolog");
}

static int run_attacklogic_extendtalents2_shenshuilog_harness(lua_State *L)
{
  return run_attacklogic_extendtalents2_fullprobe_harness_impl(L, 0, 0, 0, 0, 0, 0, 1, 0);
}
...
int main(int argc, char **argv)
{
  ...
  if (mode != NULL &&
      (strcmp(mode, "getrrevrole") == 0 ||
       ...
       strcmp(mode, "attacklogic_extendtalents2_shieldlog") == 0 ||
       strcmp(mode, "attacklogic_extendtalents2_shieldlog_direct") == 0 ||
       ...
       strcmp(mode, "attacklogic_extendtalents2_xiaoyaolog") == 0 ||
       strcmp(mode, "attacklogic_extendtalents2_shenshuilog") == 0 ||
    ...
    } else if (strcmp(mode, "attacklogic_extendtalents2_shieldlog") == 0) {
      status = run_attacklogic_extendtalents2_shieldlog_harness(L);
    } else if (strcmp(mode, "attacklogic_extendtalents2_shieldlog_direct") == 0) {
      status = run_attacklogic_extendtalents2_shieldlog_direct_harness(L);
    ...
    } else if (strcmp(mode, "attacklogic_extendtalents2_xiaoyaolog") == 0) {
      status = run_attacklogic_extendtalents2_xiaoyaolog_harness(L);
    } else if (strcmp(mode, "attacklogic_extendtalents2_shenshuilog") == 0) {
      status = run_attacklogic_extendtalents2_shenshuilog_harness(L);
